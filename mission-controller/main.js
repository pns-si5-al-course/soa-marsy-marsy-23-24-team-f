...
async function main() {
  ...
  try {
    ...
    if (rocketStatus.status === "GO") {
      ...
      console.log(
        "Richard : demande au département fusée de charger le payload"
      );
      console.log("-- POST rocket-service:3001/rocket/load --");
      const rocketLoaded = await post(
        rocketDeptServiceUrl + "/rocket/load"
      ).then((r) => {
        console.log(r);
    ...
    if (status.rocketReady && status.weatherReady) {
      ...
      const rocketLaunched = await post(
        rocketDeptServiceUrl + "/rocket",
        rocketStatus
      )
        .then((response) => {
      ...
      const telemetrieInterval = setInterval(async () => {
        const telemetrics = await get(
          telemetrieServiceUrl + "/rocket/telemetrics"
        ).then((response) => {
        ...
        const payloadTelemetrics = await get(
          payloadServiceUrl + "/rocket/payload/data"
        ).then((response) => {
          if (process.stdout.isTTY) {
            process.stdout.clearLine();
            process.stdout.cursorTo(0);
